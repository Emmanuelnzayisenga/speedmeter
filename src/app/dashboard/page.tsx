...
import { formatSpeed, formatCurrency, VEHICLE_TYPE_ICONS } from '@/lib/utils'
import {
  Activity, Car, AlertTriangle, Gauge, DollarSign,
  MapPin, RefreshCw, Navigation, Clock, X
} from 'lucide-react'
  ...
  return (
    <AppLayout>
      <div className="flex flex-col h-full">
        ...
        {/* ── Ticker bar ─────────────────────────────────────────────────────── */}
        <div className="h-10 bg-sw-nav border-b border-border/50 flex items-center overflow-hidden flex-shrink-0">
          ...
          <div className="ticker-wrap flex-1 px-4 overflow-hidden">
            <div className="ticker-content flex items-center gap-8 text-[10px] font-mono text-muted-foreground">
              {[...vehicles, ...vehicles].map((v, i) => (
                <span key={`${v.id}-${i}`} className={cn(v.status === 'SPEEDING' && 'text-sw-danger')}>
          ...
          <div className="flex-shrink-0 px-3 border-l border-border/50 h-full flex items-center gap-2">
            ...
            <RefreshCw
              className="w-3 h-3 text-muted-foreground cursor-pointer hover:text-primary transition-colors ml-1"
              onClick={refetch}
            />
            {error && (
              <span className="text-[10px] text-sw-danger font-mono ml-1">⚠ {error}</span>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 flex min-h-0">
              ...
              <div className="absolute top-6 right-6 w-72 animate-slide-in-right">
                <div className="bg-card/95 backdrop-blur-sm border border-border rounded-lg shadow-xl overflow-hidden">
                  ...
                  <div className="divide-y divide-border/30 max-h-52 overflow-y-auto">
                    {data.recentViolations.slice(0, 4).map((v: any) => (
                      <div
                        key={v.id}
                        className="px-3 py-2 flex items-center gap-2.5 hover:bg-secondary/30 transition-colors"
                      >
          ...
          {selectedVehicle && (
            <div className="w-72 border-l border-border/50 flex-shrink-0 bg-sw-panel/50 flex-col animate-slide-in-right hidden lg:flex">
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-4">
                ...
                <div className="flex items-center gap-3">
                  <div
                    ...
                    style={{
                      backgroundColor: `${selectedVehicle.color || '#3B82F6'}20`,
                      border: `1px solid ${selectedVehicle.color || '#3B82F6'}40`,
                    }}
                  >
                  ...
                  <div>
                    ...
                    <Badge
                      variant={
                        selectedVehicle.status === 'SPEEDING' ? 'destructive' :
                        selectedVehicle.status === 'MOVING'   ? 'radar' : 'success'
                      }
